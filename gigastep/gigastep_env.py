...
class GigastepEnv:
    ...
    def get_observation(self, states, has_detected, took_damage, rng, agent_id):
        ...
        if "vector" in self._obs_type:
            ...
            # ).astype(jnp.float32)
            # #
            # # relative_x_in_map = (self.limits[0] - agent_states["x"]) * (agent_states["team"] == 1) \
            #
            # agent_states_x_inverse = self.limits[0] - agent_states["x"]
            # agent_states_y_inverse = self.limits[1] - agent_states["y"]
            # agent_states_heading = jnp.fmod(agent_states["heading"] + jnp.pi, 2 * jnp.pi) - jnp.pi
            #
            # relative_x = (agent_states["x"] - agent_states["x"][agent_id]) * (agent_states["team"] == 0) \
            #             + (agent_states_x_inverse - agent_states_x_inverse[agent_id]) * (agent_states["team"] == 1)
            # relative_y = (agent_states["y"] - agent_states["y"][agent_id]) * (agent_states["team"] == 0) \
            #             + (agent_states_y_inverse - agent_states_y_inverse[agent_id]) * (agent_states["team"] == 1)
            #
            # relative_z = agent_states["z"] - agent_states["z"][agent_id]
            ...
            #      - agent_states["heading"][agent_id]
            # ) * (agent_states["team"] == 0) \
            # + (agent_states_heading - agent_states_heading[agent_id]) \
            #         * (agent_states["team"] == 1)
            #
            #
            # relative_v = agent_states["v"] - agent_states["v"][agent_id]
            ...
            #
            # # Overwrite the first element with the absolute value of the ego agent
            # relative_x = relative_x.at[agent_id].set(agent_states_x_inverse[agent_id] * \
            #                                          (agent_states["team"][agent_id] == 0) \
            #                                          + agent_states["x"][agent_id] \
            #                                          * (agent_states["team"][agent_id]  == 1)
            #                                          )
            # relative_y = relative_y.at[agent_id].set(agent_states_y_inverse[agent_id] * \
            #                                          (agent_states["team"][agent_id]  == 0) \
            ...
            # relative_z = relative_z.at[agent_id].set(agent_states["z"][agent_id])
            # relative_heading = relative_heading.at[agent_id].set(
            #     agent_states["heading"][agent_id] * (agent_states["team"][agent_id]  == 0) \
            #     + agent_states_heading[agent_id] * (agent_states["team"][agent_id]  == 1)
            # )
            #
            # relative_v = relative_v.at[agent_id].set(agent_states["v"][agent_id])
            #
    ...
    def reset(self, rng):
        ...
        # team1 = self._maps["team"][map_idx]
        # team2 = 1-self._maps["team"][map_idx]
