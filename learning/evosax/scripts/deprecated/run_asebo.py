...
from evosax import ASEBO, ParameterReshaper
...
def run_gigastep_fitness(
  ...
  network_type: str = "LSTM_CNN"
    ...
    if log_tensorboard:
        ...
        logdir = './logdir/evosax/asebo/' + datetime.now().strftime("%y_%m_%d_%H_%M_%S")
    ...
    strategy = ASEBO(popsize=6, num_dims=train_param_reshaper.total_params, maximize=True)
    ...
    t = tqdm.tqdm(range(1, num_generations + 1), desc="ASEBO", leave=True)
    for gen in t:
