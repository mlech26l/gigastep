...
def run_gigastep_fitness(
  ...
  network_type: str = "LSTM_CNN"
    ...
    if log_tensorboard:
        ...
        logdir = './logdir/evosax/des/' + datetime.now().strftime("%y_%m_%d_%H_%M_%S")
    ...
    strategy = DES(popsize=30, num_dims=train_param_reshaper.total_params, maximize=True)
    ...
    t = tqdm.tqdm(range(1, num_generations + 1), desc="DES", leave=True)
    for gen in t:
